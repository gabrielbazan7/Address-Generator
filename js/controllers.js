...
$('#selectN').change(function(){
	...
	$scope.generate = function(){
		...
		var validation = generatorServices.valGenerate(backUps, passwords, m, n);
		...
		if(validation == true){
			...
			generatorServices.getActiveAddresses(passwords, backUps, mainPath, n, function(mainAddressesObject) {
				if(mainAddressesObject.length > 0){
					...
					for(var i=0; i<mainAddressesObject.length ;i++){
						...
						for(var j=0; j<mainAddressesObject[i].utxoData.utxo.length ;j++){
							utxos.push(mainAddressesObject[i].utxoData.utxo[j]);
							privateKeys.push(mainAddressesObject[i].utxoData.privKeys[j]);
						}	
				...
				generatorServices.getActiveAddresses(passwords, backUps, changePath, n, function(changeAddressesObject) {
					...
					if(changeAddressesObject.length > 0){
						...
						for(var n=0; n<changeAddressesObject.length ;n++){
							...
							for(var k=0; k<changeAddressesObject[n].utxoData.utxo.length ;k++){
							utxos.push(changeAddressesObject[n].utxoData.utxo[k]);
							privateKeys.push(changeAddressesObject[n].utxoData.privKeys[k]);
						}	
	...
	$scope.send = function(){
		...
		privateKeys = lodash.remove(privateKeys,function (n){
			return !lodash.isUndefined(n);
		});
		if(validation == true){
			$scope.textArea += 'Creating transaction to retrieve total amount...\n\n';
			var rawTx = generatorServices.createRawTx(addr, utxos, totalBalance, privateKeys,n);
